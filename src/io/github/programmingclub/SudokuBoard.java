...
public class SudokuBoard {
	 ...
	 *            the value
	 * @param permananat
	 *            true if it is being read from the file
	 */
	...
	public void removePossibilitiesFromGrid(Integer index, Integer value) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (grid.get(firstIndexOfGrid + ((i * 9) + j))
						.getPossibilities().contains(value)) {
					grid.get(firstIndexOfGrid + ((i * 9) + j))
	...
	public void removePossibilitiesFromRow(Integer index, Integer value) {
		...
		for (int i = 0; i < 9; i++) {
			...
			if (grid.get(firstIndexOfRow + i).getPossibilities()
					.contains(value)) {
	...
	public void removePossibilitiesFromColumn(Integer index, Integer value) {
		...
		for (int i = 0; i < 9; i++) {
			if (grid.get(firstIndexOfColumn + (i * 9)).getPossibilities()
					.contains(value)) {
				grid.get(firstIndexOfColumn + (i * 9)).getPossibilities()
				.remove(value);
			}
		}
	}
	...
	public void solveLogically() {
		...
		for (int i = 0; i < 81; i++) {
			if (grid.get(i).getPossibilities().size() == 1) {
				addValueToIndex(i, grid.get(i).getPossibilities().getFirst(),
						true);
				changed = true;
			}
		}
	...
	/**
	 * It searches through the nodes and of the board and returns the index of
	 * the empty nodes in an ArrayList
	 * 
	...
	public void solveBruteForce() {
		...
		while (emptyNodeArrayIndex < emptyNodes.size()) {
			...
			if (value == 9) {
				...
				if (rowCheckConsistency(emptyNodeIndex)
						&& columnCheckConsistency(emptyNodeIndex)
	 ...
	 * 
	 * @param index
	 *            of the row that should be checked for consistency
	 * @return true if it is consistent
	 */
	public boolean rowCheckConsistency(Integer index) {
		int firstIndexOfRow = findRowFirstIndex(index);
		for (int i = 0; i < 9; i++) {
			if (grid.get(firstIndexOfRow + i).getValue() == grid.get(index)
					.getValue() && (firstIndexOfRow + i) != index) {
				return false;
			}
		}
		return true;
	 ...
	 * 
	 * @param index
	 *            of the column that should be checked for consistency
	 * @return true if it is consistent
	 */
	public boolean columnCheckConsistency(Integer index) {
		int firstIndexOfColumn = findColumnFirstIndex(index);
		for (int i = 0; i < 9; i++) {
			if (grid.get(firstIndexOfColumn + (i * 9)).getValue() == grid.get(
					index).getValue()
					&& (firstIndexOfColumn + (i * 9) != index)) {
				return false;
			}
		}
		return true;
	 ...
	 * 
	 * @param index
	 *            of the grid that should be checked for consistency
	 * @return true if it is consistent
	 */
	public boolean gridCheckConsistency(Integer index) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (grid.get(firstIndexOfGrid + ((i * 9) + j)).getValue() == grid
						.get(index).getValue()
						&& (firstIndexOfGrid + ((i * 9) + j)) != index) {
					return false;
				}
			}
		}
